...
def AssertP_End(fileName):
    ...
    for SboxTag in range(32):
        ...
        command = command + "0bin000000000@" + SName("P", 4, 0, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "0bin000000000@" + SName("P", 4, 1, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "0bin000000000@" + SName("P", 5, 0, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "0bin000000000@" + SName("P", 5, 1, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "0bin000000000@" + SName("P", 6, 0, SboxTag) + ","
...
def main(fileName, p, ActLocArray, ActValue):
    ...
    AssertVariable(fileName, "S", 5, 1, 6, 32)
    AssertVariable(fileName, "S", 5, 2, 6, 32)
    AssertVariable(fileName, "S", 5, 3, 6, 32)
    AssertVariable(fileName, "S", 5, 4, 6, 32)

    AssertVariable(fileName, "S", 6, 0, 6, 32)

    AssertVariable(fileName, "P", 4, 0, 32, 15)
    AssertVariable(fileName, "P", 4, 1, 32, 15)

    AssertVariable(fileName, "P", 5, 0, 32, 15)
    AssertVariable(fileName, "P", 5, 1, 32, 15)

    AssertVariable(fileName, "P", 6, 0, 32, 15)

    AssertP_Begin(fileName)

    DDTCVCprint(fileName, DDT)

    Differential_MCCVCPrint(fileName, "S", 3, 4, DifferentialM)

    SboxCVCPrint_P_Print(fileName, "DDT", "S", 4, 0, 32, "P", 0)
    SCCVCPrint(fileName, "S", 4, 1)
    SboxCVCPrint_P_Print(fileName, "DDT", "S", 4, 2, 32, "P", 1)
    SCCVCPrint(fileName, "S", 4, 3)
    Differential_MCCVCPrint(fileName, "S", 4, 4, DifferentialM)

    SboxCVCPrint_P_Print(fileName, "DDT", "S", 5, 0, 32, "P", 0)
    SCCVCPrint(fileName, "S", 5, 1)
    SboxCVCPrint_P_Print(fileName, "DDT", "S", 5, 2, 32, "P", 1)
    SCCVCPrint(fileName, "S", 5, 3)
    Differential_MCCVCPrint(fileName, "S", 5, 4, DifferentialM)



    SetSboxValue(fileName, "S", 3, 4, ActLocArray, ActValue)

    AssertActSboxP(fileName, "S", 6, 0, "P", 0)
...
fileName = "SPEEDY_Differential_SecondTrail.cvc"
p = 140
ActLoc = [0]
ActValue = "0b000010"
