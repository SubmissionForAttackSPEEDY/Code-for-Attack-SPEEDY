...
def AssertP_End(fileName):
    ...
    for SboxTag in range(32):
        ...
        command = command + "0bin000000000@" + SName("P", 0, 1, SboxTag) + ","

    for SboxTag in range(32):

        command = command + "0bin000000000@" + SName("P", 1, 0, SboxTag) + ","
...
def AssertSboxNum(fileName, VarName, round, Tag, PName, PTag):

    for i in range(32):
        command = "IF " + SboxName(VarName, round, Tag, i) + " = 0bin000000 THEN " + SName(PName, round, PTag,
                                                                                          i) + " = 0bin00000000"

        command = command + " ELSE " + SName(PName, round, PTag, i) + " = 0bin00000001 ENDIF"
...
def Assert_SboxNum_2_2_Begin(fileName):
    AssertPrint(fileName, "SboxNum_2_2:BITVECTOR(8);")

def Assert_SboxNum_0_4_Begin(fileName):

    AssertPrint(fileName, "SboxNum_0_4:BITVECTOR(8);")


def Assert_SboxNum_0_4_BVLE(fileName):


    command = "ASSERT(BVLE(SboxNum_0_4, 0hex0f));"

    AssertPrint(fileName, command)

def Assert_SboxNum_2_2_BVLE(fileName):


    command = "ASSERT(BVLE(0hex06, SboxNum_2_2));"
...
def AssertState_i(fileName, VarName, round, Tag, SboxTag, ActValue):

    command = SboxName(VarName, round, Tag, SboxTag) + " = " + ActValue
    command = AssertCommand(command)
    AssertPrint(fileName, command)

def AssertState(fileName, VarName, round, Tag, ActArray, ActValue):

    for SboxTag in range(32):

        if SboxTag in ActArray:
            AssertState_i(fileName, VarName, round, Tag, SboxTag, ActValue)

        else:
            AssertState_i(fileName, VarName, round, Tag, SboxTag, "0bin000000")

def Assert_SboxNum_0_4_End(fileName):

    command = "SboxNum_0_4 =  BVPLUS(8,"

    # p_0_0, Differential
    for SboxTag in range(32):
            command = command + SName("SboxNum", 0, 4, SboxTag) + ","
...
def Assert_SboxNum_2_2_End(fileName):
    command = "SboxNum_2_2 =  BVPLUS(8,"
...
def main(fileName, p, SboxLocFirst, SboxValueFirst, SboxLocSecond, SboxValueSecond):

    M = [0, 1, 5, 9, 15, 21, 26]
    ClearFile(fileName)

    # Key Recovery round, without S_0_0
    AssertVariable(fileName, "S", 0, 1, 6, 32)
    AssertVariable(fileName, "S", 0, 2, 6, 32)
    AssertVariable(fileName, "S", 0, 3, 6, 32)
    AssertVariable(fileName, "S", 0, 4, 6, 32)

    # First round
    AssertVariable(fileName, "S", 1, 0, 6, 32)
    AssertVariable(fileName, "S", 1, 1, 6, 32)
    AssertVariable(fileName, "S", 1, 2, 6, 32)
    ...
    # Key recovery p from FTDDT
    AssertVariable(fileName, "P", 0, 1, 32, 15)

    # P from first round
    AssertVariable(fileName, "P", 1, 0, 32, 15)
    ...
    # Assert SboxNum
    AssertVariable(fileName, "SboxNum", 0, 1, 32, 8)
    AssertVariable(fileName, "SboxNum", 2, 2, 32, 8)

    AssertVariable(fileName, "SboxNum", 0, 4, 32, 8)

    # Assert P
    AssertP_Begin(fileName)
    AssertSboxNum_Begin(fileName)
    Assert_SboxNum_0_4_Begin(fileName)
    Assert_SboxNum_2_2_Begin(fileName)

    # Assert Table
    DDTCVCprint(fileName, DDT)
    FTDDTprint(fileName, FTDDT)

    # Begin to explain trail

    # key recovery round
    SCCVCPrint(fileName, "S", 0, 1)
    SboxCVCPrint_P_Print(fileName, "FTDDT", "S", 0, 2, 32, "P", 1)
    SCCVCPrint(fileName, "S", 0, 3)
    Differential_MCCVCPrint(fileName, "S", 0, 4, M)

    # First round
    SboxCVCPrint_P_Print(fileName, "DDT", "S", 1, 0, 32, "P", 0)
    SCCVCPrint(fileName, "S", 1, 1)
    SboxCVCPrint_P_Print(fileName, "DDT", "S", 1, 2, 32, "P", 1)
    ...
    # Assert SboxNum
    AssertSboxNum(fileName, "S", 0, 1, "SboxNum", 1)
    AssertSboxNum(fileName, "S", 2, 2, "SboxNum", 2)

    AssertSboxNum(fileName, "S", 0, 4, "SboxNum", 4)

    AssertState(fileName, "S", 1, 4, SboxLocFirst, SboxValueFirst)
    AssertState(fileName, "S", 2, 0, SboxLocSecond, SboxValueSecond)

    AssertP_End(fileName)
    AssertSboxNum_End(fileName)

    Assert_SboxNum_0_4_End(fileName)
    Assert_SboxNum_2_2_End(fileName)

    Assert_BVLE(fileName, p)
    Assert_SboxNum_BVLE(fileName)
    Assert_SboxNum_0_4_BVLE(fileName)
    Assert_SboxNum_2_2_BVLE(fileName)

    EndPrint(fileName)



fileName = "SPEEDY_Differential_KeyRecovery.cvc"
p = 35

SboxLocFirst = [8]
SboxValueFirst = "0bin000001"
SboxLocSecond = [3,6,9,15,18,27,28]
SboxValueSecond = "0bin000010"

main(fileName, p, SboxLocFirst, SboxValueFirst, SboxLocSecond, SboxValueSecond)
