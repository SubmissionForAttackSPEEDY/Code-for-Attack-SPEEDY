...
def AssertP_END(fileName):
    ...
    for SboxTag in range(32):
        command = command + "  0bin000000000000@" + SName("P", 0, 1, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "  0bin000000000000@" + SName("P", 1, 0, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "  0bin000000000000@" + SName("P", 1, 1, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "  0bin000000000000@" + SName("P", 2, 0, SboxTag) + ","

    for SboxTag in range(32):
        command = command + "  0bin000000000000@" + SName("P", 2, 1, SboxTag) + ","
...
def main(fileName, p, SboxLoc, SboxValue):
    ...
    AssertVariable(fileName, "P", 0, 1, 32, 12)

    AssertVariable(fileName, "P", 1, 0, 32, 12)
    AssertVariable(fileName, "P", 1, 1, 32, 12)

    AssertVariable(fileName, "P", 2, 0, 32, 12)
    AssertVariable(fileName, "P", 2, 1, 32, 12)
    ...
    MCCVCPrint(fileName, "S", 0, 4, M)

    SboxCVCPrint_LAT_P_Print(fileName, "S", 1, 0, 32, "P", 0)
    SCCVCPrint(fileName, "S", 1, 1)
    SboxCVCPrint_LAT_P_Print(fileName, "S", 1, 2, 32, "P", 1)
    SCCVCPrint(fileName, "S", 1, 3)
    MCCVCPrint(fileName, "S", 1, 4, M)

    SboxCVCPrint_LAT_P_Print(fileName, "S", 2, 0, 32, "P", 0)
    SCCVCPrint(fileName, "S", 2, 1)
    SboxCVCPrint_LAT_P_Print(fileName, "S", 2, 2, 32, "P", 1)
    SCCVCPrint(fileName, "S", 2, 3)
    MCCVCPrint(fileName, "S", 2, 4, M)

    AssertActRowP(fileName, "S", 0, 3, "P", 1)
...
fileName = "SPEEDY_Linear_FirstTrail.cvc"
p = 50
SboxLoc = [0, 11]
